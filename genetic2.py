...
def time_phase(start_h, end_h, start_m, end_m, include_start=True, include_end=True):
    sesh = []
    for i in range(start_h, end_h + 1):
        if i == end_h:
            upper = end_m + 1 if include_end else end_m
            for j in range(0, upper, 5):
                sesh.append(f'{i:02d}:{j:02d}')
        elif i == start_h:
            lower = start_m if include_start else start_m + 5
            for j in range(lower, 59, 5):
                sesh.append(f'{i:02d}:{j:02d}')
        else:
...
def day_unavailable():
    ...
    for unavailable_slot in unavailable_slots:
        begin_hour, begin_minute = map(int, unavailable_slot[1].split(':'))
        end_hour, end_minute = map(int, unavailable_slot[2].split(':'))
 
        if begin_hour > end_hour:
            unavailable_day = time_phase(0, end_hour, 0, end_minute)[:-1]
            unavailable_night = time_phase(begin_hour, 23, begin_minute, 59)[1:]
...
def random_gene_generation(task_id, session_length, dates_before_due, real_due): # i want to input the datetime type 
    while True:
        ...
        if end <= real_due and end.strftime('%H:%M') in DAY_AVAIL_SLOTS:
...
def generating_minutes(working_hours, max_working):
    ...
    while working_hours > 0:
        if working_hours >= max_working:
            ...
            session_length = random.randrange(15, working_hours, 5)
            sessions.append(session_length)
            working_hours -= session_length
...
def genetic_algor(population_size, generations, mutation_rate):
    ...
    now = datetime.now()
    CEIL = now + (timedelta(minutes=5) - timedelta(minutes=now.minute % 5, seconds=now.second, microseconds=now.microsecond)) % timedelta(minutes=5)
